...
export function useReferenceImage(
  ...
  // 1-3: synchronous, instant — no flash, no network.
  ...
  const { url: taxaPhotoUrl, isResolving, source: externalSource } = useTaxaPhoto(
    ...
    needsExternal ? buildResolverIdentity(input) : null,
  )

  const uri = preResolved?.uri ?? taxaPhotoUrl
  ...
  useReferenceImagePipelineLog({ input, debug, preResolvedSource: preResolved?.source ?? null, taxaPhotoUrl, externalSource, uri, source, isResolving })

  return { uri, source, isResolving }
}
...
function useReferenceImagePipelineLog(args: PipelineLogArgs): void {
  ...
  useEffect(() => {
    ...
    console.log('REFERENCE IMAGE PIPELINE', {
      ...
      wikipediaImage: externalSource === 'wikipedia' ? taxaPhotoUrl : null,
      googleImage: null,
