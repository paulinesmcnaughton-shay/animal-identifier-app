import { NOCTURNAL_SPECIES_KEYWORDS } from '@/features/achievements/badge-earned'
...
const TYPE_ORDER: QuestType[] = ['insect', 'bird', 'reptile', 'venue', 'mammal', 'dusk', 'flower', 'water', 'streak']
...
const TYPE_META: Record<QuestType, TypeMeta> = {
  ...
  dusk:    { emoji: '🌙', label: 'NOCTURNAL', verb: 'Spot',    noun: 'nocturnal animals', targets: [1, 3, 6, 10, 18], xp: [90, 140, 220, 300, 380] },
...
function questTitle(type: QuestType, target: number): string {
  ...
  if (type === 'dusk' && target === 1) return 'Spot a nocturnal animal'
...
/** A nocturnal species (matched by name) — what the Dusk quest counts. */
function isNocturnal(s: SightingLike): boolean {
  const text = `${s.speciesName ?? ''} ${s.scientificName ?? ''} ${s.speciesId ?? ''}`.toLowerCase()
  return NOCTURNAL_SPECIES_KEYWORDS.some((k) => text.includes(k))
}
...
export function questTypesForSighting(s: SightingLike, lookup?: CollectionLookup): Set<QuestType> {
  ...
  if (isNocturnal(s)) types.add('dusk')
  if (lookup) {
...
const QUEST_TYPES: QuestType[] = ['insect', 'bird', 'reptile', 'venue', 'mammal', 'dusk', 'flower', 'water', 'streak']
 ...
 * twice counts once); the streak type reflects streak_days.
 */
